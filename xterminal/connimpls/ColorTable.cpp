...
void VTColorTable::InitXterm256Colors()
{
    for (int i = 0; i < 256; ++i) {
        mTbl[i] = Color((kXterm256Colors[i] >> 16) & 0xff,
                        (kXterm256Colors[i] >> 8 ) & 0xff,
                        (kXterm256Colors[i] & 0xff));
    }
}

void VTColorTable::SetColor(int color_id, const Upp::Color& color)
{
...
int VTColorTable::FindNearestColorId(int idx)
{
    return idx % 255;
}
