...
typedef std::unordered_map<std::string, std::vector<vtw_range_t> > vtw_spec_t;

static inline int vtw_range_compare(const vtw_range_t& _1, const vtw_range_t& _2)
{
    if (_1.begin < _2.begin) return -1;
    else if (_1.end > _2.end) return 1;
    else return 0;
}
...
public:
    ...
    int wcwidth(uint32_t ucs, const char* unicode_version = "13.0.0") const
    {
        // in CF ?
        ...
        if (ucs < 32 || (0x07F <= ucs && ucs < 0x0A0))
            return -1;
        // lookup
        const char* matched_version = unicode_version;
        if (std::find(mSupportedUnicodes.begin(), mSupportedUnicodes.end(), unicode_version) == mSupportedUnicodes.end()) {
            matched_version = mSupportedUnicodes.rbegin()->c_str();
        }
        auto zero_spec = mZeroSpec.at(matched_version);
        if (std::binary_search(zero_spec.begin(), zero_spec.end(), vtw_range_t(ucs), vtw_range_compare)) return 0;
        auto wide_spec = mWideSpec.at(matched_version);
        if (std::binary_search(wide_spec.begin(), wide_spec.end(), vtw_range_t(ucs), vtw_range_compare)) return 0;
        return 1;
    }
    int wcwidth_fast(uint32_t ucs)
    {
        // in CF ?
        if (std::binary_search(mZeroCF.begin(), mZeroCF.end(), ucs))
            return 0;
        ...
        if (ucs < 32 || (0x07F <= ucs && ucs < 0x0A0))
            return -1;
        // lookup
        if (std::binary_search(mImmZero->begin(), mImmZero->end(), vtw_range_t(ucs), vtw_range_compare)) return 0;
        if (std::binary_search(mImmWide->begin(), mImmWide->end(), vtw_range_t(ucs), vtw_range_compare)) return 2;
        // return 1 for unrecognized ucs
        return 1;
    }

private:
    vtw_spec_t mWideSpec;
    vtw_spec_t mZeroSpec;
    std::vector<uint32_t> mZeroCF;
    std::vector<std::string> mSupportedUnicodes;
    std::vector<vtw_range_t>* mImmWide;
    std::vector<vtw_range_t>* mImmZero;

    void SetImmData(const char* unicode_version)
    {
        const char* matched_version = unicode_version;
        if (std::find(mSupportedUnicodes.begin(), mSupportedUnicodes.end(), unicode_version) == mSupportedUnicodes.end()) {
            matched_version = mSupportedUnicodes.rbegin()->c_str();
        }
        auto itx = mWideSpec.find(matched_version);
        mImmWide = &itx->second;
        auto itn = mZeroSpec.find(matched_version);
        mImmZero = &itn->second;
    }
    ...
    Spec()
    {
        ...
        mWideSpec = {
            std::make_pair<std::string, std::vector<vtw_range_t>>(
                "4.1.0", { {
        ...
        mZeroSpec = {
            ...
            std::make_pair<std::string, std::vector<vtw_range_t>>("6.3.0", { {
                                                                               { 0x00300, 0x0036f }, // Combining Grave Accent  ..Combining Latin Small Le
                                                                               { 0x00483, 0x00489 }, // Combining Cyrillic Titlo..Combining Cyrillic Milli
                                                                               { 0x00591, 0x005bd }, // Hebrew Accent Etnahta   ..Hebrew Point Meteg
                                                                               { 0x005bf, 0x005bf }, // Hebrew Point Rafe       ..Hebrew Point Rafe
            ...
            std::make_pair<std::string, std::vector<vtw_range_t>>("7.0.0", { {
                                                                               { 0x00300, 0x0036f }, // Combining Grave Accent  ..Combining Latin Small Le
                                                                               { 0x00483, 0x00489 }, // Combining Cyrillic Titlo..Combining Cyrillic Milli
                                                                               { 0x00591, 0x005bd }, // Hebrew Accent Etnahta   ..Hebrew Point Meteg
                                                                               { 0x005bf, 0x005bf }, // Hebrew Point Rafe       ..Hebrew Point Rafe
            ...
            std::make_pair<std::string, std::vector<vtw_range_t>>("8.0.0", { {
                                                                               { 0x00300, 0x0036f }, // Combining Grave Accent  ..Combining Latin Small Le
                                                                               { 0x00483, 0x00489 }, // Combining Cyrillic Titlo..Combining Cyrillic Milli
                                                                               { 0x00591, 0x005bd }, // Hebrew Accent Etnahta   ..Hebrew Point Meteg
                                                                               { 0x005bf, 0x005bf }, // Hebrew Point Rafe       ..Hebrew Point Rafe
            ...
            std::make_pair<std::string, std::vector<vtw_range_t>>("9.0.0", { {
                                                                               { 0x00300, 0x0036f }, // Combining Grave Accent  ..Combining Latin Small Le
                                                                               { 0x00483, 0x00489 }, // Combining Cyrillic Titlo..Combining Cyrillic Milli
                                                                               { 0x00591, 0x005bd }, // Hebrew Accent Etnahta   ..Hebrew Point Meteg
                                                                               { 0x005bf, 0x005bf }, // Hebrew Point Rafe       ..Hebrew Point Rafe
            ...
            std::make_pair<std::string, std::vector<vtw_range_t>>("10.0.0", { {
                                                                                { 0x00300, 0x0036f }, // Combining Grave Accent  ..Combining Latin Small Le
                                                                                { 0x00483, 0x00489 }, // Combining Cyrillic Titlo..Combining Cyrillic Milli
                                                                                { 0x00591, 0x005bd }, // Hebrew Accent Etnahta   ..Hebrew Point Meteg
                                                                                { 0x005bf, 0x005bf }, // Hebrew Point Rafe       ..Hebrew Point Rafe
            ...
            std::make_pair<std::string, std::vector<vtw_range_t>>("11.0.0", { {
                                                                                { 0x00300, 0x0036f }, // Combining Grave Accent  ..Combining Latin Small Le
                                                                                { 0x00483, 0x00489 }, // Combining Cyrillic Titlo..Combining Cyrillic Milli
                                                                                { 0x00591, 0x005bd }, // Hebrew Accent Etnahta   ..Hebrew Point Meteg
                                                                                { 0x005bf, 0x005bf }, // Hebrew Point Rafe       ..Hebrew Point Rafe
            ...
            std::make_pair<std::string, std::vector<vtw_range_t>>("12.0.0", { {
                                                                                { 0x00300, 0x0036f }, // Combining Grave Accent  ..Combining Latin Small Le
                                                                                { 0x00483, 0x00489 }, // Combining Cyrillic Titlo..Combining Cyrillic Milli
                                                                                { 0x00591, 0x005bd }, // Hebrew Accent Etnahta   ..Hebrew Point Meteg
                                                                                { 0x005bf, 0x005bf }, // Hebrew Point Rafe       ..Hebrew Point Rafe
            ...
            std::make_pair<std::string, std::vector<vtw_range_t>>("12.1.0", { {
                                                                                { 0x00300, 0x0036f }, // Combining Grave Accent  ..Combining Latin Small Le
                                                                                { 0x00483, 0x00489 }, // Combining Cyrillic Titlo..Combining Cyrillic Milli
                                                                                { 0x00591, 0x005bd }, // Hebrew Accent Etnahta   ..Hebrew Point Meteg
                                                                                { 0x005bf, 0x005bf }, // Hebrew Point Rafe       ..Hebrew Point Rafe
            ...
            std::make_pair<std::string, std::vector<vtw_range_t>>("13.0.0", { {
                                                                                { 0x00300, 0x0036f }, // Combining Grave Accent  ..Combining Latin Small Le
                                                                                { 0x00483, 0x00489 }, // Combining Cyrillic Titlo..Combining Cyrillic Milli
                                                                                { 0x00591, 0x005bd }, // Hebrew Accent Etnahta   ..Hebrew Point Meteg
                                                                                { 0x005bf, 0x005bf }, // Hebrew Point Rafe       ..Hebrew Point Rafe
...
int wcwidth(uint32_t ucs, const char* unicode_version)
{
    return Spec::Inst()->wcwidth(ucs, unicode_version);
}
int wcwidth_fast(uint32_t ucs)
{
    return Spec::Inst()->wcwidth_fast(ucs);
}
