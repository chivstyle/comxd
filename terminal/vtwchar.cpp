...
struct vtw_range_t {
    uint32_t begin;
    uint32_t end;
    vtw_range_t(uint32_t ucs)
        : begin(ucs)
        , end(ucs)
    {
    }
    vtw_range_t(uint32_t begin_, uint32_t end_)
        : begin(begin_)
        , end(end_)
    {
    }
};
static inline int vtw_range_compare(const vtw_range_t& a, const vtw_range_t& b)
{
    // a = b, return false;
    if (a.begin >= b.begin && a.end <= b.end ||
        b.begin >= a.begin && b.end <= a.end) return false;
    // a < b return true
    if (a.end < b.begin) return true;
...
public:
    ...
    {
        // in CF ?
        if (std::binary_search(mZeroCF.begin(), mZeroCF.end(), ucs))
            return 0;
        // C0/C1
        if (ucs < 32 || (0x07F <= ucs && ucs < 0x0A0))
            return -1;
        // lookup
        if (std::binary_search(mZeroSpec.begin(), mZeroSpec.end(), vtw_range_t(ucs), vtw_range_compare))
            return 0;
        if (std::binary_search(mWideSpec.begin(), mWideSpec.end(), vtw_range_t(ucs), vtw_range_compare))
            return 2;
        // return 1 for others
        return 1;
    }
...
private:
    std::vector<vtw_range_t> mWideSpec; // up to Unicode 13.0
    std::vector<vtw_range_t> mZeroSpec; // up to Unicode 13.0
    std::vector<uint32_t> mZeroCF;
    ...
    Spec()
    {
        ...
        mWideSpec = std::vector<vtw_range_t>({
            { 0x01100, 0x0115f }, // Hangul Choseong Kiyeok  ,Hangul Choseong Filler
            { 0x0231a, 0x0231b }, // Watch                   ,Hourglass
            { 0x02329, 0x0232a }, // Left-pointing Angle Brac,Right-pointing Angle Bra
            { 0x023e9, 0x023ec }, // Black Right-pointing Dou,0xlack Down-pointing Doub
            { 0x023f0, 0x023f0 }, // Alarm Clock             ,0xlarm Clock
            { 0x023f3, 0x023f3 }, // Hourglass With Flowing S,Hourglass With Flowing S
            { 0x025fd, 0x025fe }, // White Medium Small Squar,0xlack Medium Small Squar
            ...
            { 0x026ea, 0x026ea }, // Church                  ,0xhurch
            { 0x026f2, 0x026f3 }, // Fountain                ,0xlag In Hole
            { 0x026f5, 0x026f5 }, // Sailboat                ,Sailboat
            { 0x026fa, 0x026fa }, // Tent                    ,Tent
            { 0x026fd, 0x026fd }, // Fuel Pump               ,0xuel Pump
            ...
            { 0x0274c, 0x0274c }, // Cross Mark              ,0xross Mark
            ...
            { 0x027b0, 0x027b0 }, // Curly Loop              ,0xurly Loop
            { 0x027bf, 0x027bf }, // Double Curly Loop       ,0xouble Curly Loop
            ...
            { 0x02e80, 0x02e99 }, // Cjk Radical Repeat      ,0xjk Radical Rap
            { 0x02e9b, 0x02ef3 }, // Cjk Radical Choke       ,0xjk Radical C-simplified
            ...
            { 0x03105, 0x0312f }, // Bopomofo Letter B       ,0xopomofo Letter Nn
            { 0x03131, 0x0318e }, // Hangul Letter Kiyeok    ,Hangul Letter Araeae
            { 0x03190, 0x031e3 }, // Ideographic Annotation L,0xjk Stroke Q
            { 0x031f0, 0x0321e }, // Katakana Letter Small Ku,Parenthesized Korean Cha
            { 0x03220, 0x03247 }, // Parenthesized Ideograph ,0xircled Ideograph Koto
            ...
            { 0x0ff01, 0x0ff60 }, // Fullwidth Exclamation Ma,0xullwidth Right White Pa
            { 0x0ffe0, 0x0ffe6 }, // Fullwidth Cent Sign     ,0xullwidth Won Sign
            ...
            { 0x1f250, 0x1f251 }, // Circled Ideograph Advant,0xircled Ideograph Accept
            { 0x1f260, 0x1f265 }, // Rounded Symbol For Fu   ,Rounded Symbol For Cai
            { 0x1f300, 0x1f320 }, // Cyclone                 ,Shooting Star
            { 0x1f32d, 0x1f335 }, // Hot Dog                 ,0xactus
            { 0x1f337, 0x1f37c }, // Tulip                   ,0xaby Bottle
            { 0x1f37e, 0x1f393 }, // Bottle With Popping Cork,Graduation Cap
            { 0x1f3a0, 0x1f3ca }, // Carousel Horse          ,Swimmer
            { 0x1f3cf, 0x1f3d3 }, // Cricket Bat And Ball    ,Table Tennis Paddle And
            { 0x1f3e0, 0x1f3f0 }, // House Building          ,0xuropean Castle
            { 0x1f3f4, 0x1f3f4 }, // Waving Black Flag       ,Waving Black Flag
            { 0x1f3f8, 0x1f43e }, // Badminton Racquet And Sh,Paw Prints
            { 0x1f440, 0x1f440 }, // Eyes                    ,0xyes
            { 0x1f442, 0x1f4fc }, // Ear                     ,Videocassette
            { 0x1f4ff, 0x1f53d }, // Prayer Beads            ,0xown-pointing Small Red
            { 0x1f54b, 0x1f54e }, // Kaaba                   ,Menorah With Nine Branch
            { 0x1f550, 0x1f567 }, // Clock Face One Oclock   ,0xlock Face Twelve-thirty
            { 0x1f57a, 0x1f57a }, // Man Dancing             ,Man Dancing
            { 0x1f595, 0x1f596 }, // Reversed Hand With Middl,Raised Hand With Part Be
            { 0x1f5a4, 0x1f5a4 }, // Black Heart             ,0xlack Heart
            { 0x1f5fb, 0x1f64f }, // Mount Fuji              ,Person With Folded Hands
            { 0x1f680, 0x1f6c5 }, // Rocket                  ,Left Luggage
            { 0x1f6cc, 0x1f6cc }, // Sleeping Accommodation  ,Sleeping Accommodation
            { 0x1f6d0, 0x1f6d2 }, // Place Of Worship        ,Shopping Trolley
            { 0x1f6d5, 0x1f6d7 }, // Hindu Temple            ,(nil)
            { 0x1f6eb, 0x1f6ec }, // Airplane Departure      ,0xirplane Arriving
            { 0x1f6f4, 0x1f6fc }, // Scooter                 ,(nil)
            { 0x1f7e0, 0x1f7eb }, // Large Orange Circle     ,Large Brown Square
            { 0x1f90c, 0x1f93a }, // (nil)                   ,0xencer
        ...
        mZeroSpec = std::vector<vtw_range_t>({
            { 0x00300, 0x0036f }, // Combining Grave Accent  ,0xombining Latin Small Le
            { 0x00483, 0x00489 }, // Combining Cyrillic Titlo,0xombining Cyrillic Milli
            ...
            { 0x00610, 0x0061a }, // Arabic Sign Sallallahou ,0xrabic Small Kasra
            { 0x0064b, 0x0065f }, // Arabic Fathatan         ,0xrabic Wavy Hamza Below
            { 0x00670, 0x00670 }, // Arabic Letter Superscrip,0xrabic Letter Superscrip
            { 0x006d6, 0x006dc }, // Arabic Small High Ligatu,0xrabic Small High Seen
            { 0x006df, 0x006e4 }, // Arabic Small High Rounde,0xrabic Small High Madda
            { 0x006e7, 0x006e8 }, // Arabic Small High Yeh   ,0xrabic Small High Noon
            { 0x006ea, 0x006ed }, // Arabic Empty Centre Low ,0xrabic Small Low Meem
            ...
            { 0x008d3, 0x008e1 }, // Arabic Small Low Waw    ,0xrabic Small High Sign S
            { 0x008e3, 0x00902 }, // Arabic Turned Damma Belo,0xevanagari Sign Anusvara
            { 0x0093a, 0x0093a }, // Devanagari Vowel Sign Oe,0xevanagari Vowel Sign Oe
            { 0x0093c, 0x0093c }, // Devanagari Sign Nukta   ,0xevanagari Sign Nukta
            { 0x00941, 0x00948 }, // Devanagari Vowel Sign U ,0xevanagari Vowel Sign Ai
            { 0x0094d, 0x0094d }, // Devanagari Sign Virama  ,0xevanagari Sign Virama
            { 0x00951, 0x00957 }, // Devanagari Stress Sign U,0xevanagari Vowel Sign Uu
            { 0x00962, 0x00963 }, // Devanagari Vowel Sign Vo,0xevanagari Vowel Sign Vo
            { 0x00981, 0x00981 }, // Bengali Sign Candrabindu,0xengali Sign Candrabindu
            { 0x009bc, 0x009bc }, // Bengali Sign Nukta      ,0xengali Sign Nukta
            { 0x009c1, 0x009c4 }, // Bengali Vowel Sign U    ,0xengali Vowel Sign Vocal
            { 0x009cd, 0x009cd }, // Bengali Sign Virama     ,0xengali Sign Virama
            { 0x009e2, 0x009e3 }, // Bengali Vowel Sign Vocal,0xengali Vowel Sign Vocal
            { 0x009fe, 0x009fe }, // Bengali Sandhi Mark     ,0xengali Sandhi Mark
            ...
            { 0x0135d, 0x0135f }, // Ethiopic Combining Gemin,0xthiopic Combining Gemin
            { 0x01712, 0x01714 }, // Tagalog Vowel Sign I    ,Tagalog Sign Virama
            { 0x01732, 0x01734 }, // Hanunoo Vowel Sign I    ,Hanunoo Sign Pamudpod
            { 0x01752, 0x01753 }, // Buhid Vowel Sign I      ,0xuhid Vowel Sign U
            ...
            { 0x01a17, 0x01a18 }, // Buginese Vowel Sign I   ,0xuginese Vowel Sign U
            { 0x01a1b, 0x01a1b }, // Buginese Vowel Sign Ae  ,0xuginese Vowel Sign Ae
            ...
            { 0x01b00, 0x01b03 }, // Balinese Sign Ulu Ricem ,0xalinese Sign Surang
            { 0x01b34, 0x01b34 }, // Balinese Sign Rerekan   ,0xalinese Sign Rerekan
            { 0x01b36, 0x01b3a }, // Balinese Vowel Sign Ulu ,0xalinese Vowel Sign Ra R
            { 0x01b3c, 0x01b3c }, // Balinese Vowel Sign La L,0xalinese Vowel Sign La L
            { 0x01b42, 0x01b42 }, // Balinese Vowel Sign Pepe,0xalinese Vowel Sign Pepe
            { 0x01b6b, 0x01b73 }, // Balinese Musical Symbol ,0xalinese Musical Symbol
            ...
            { 0x01be6, 0x01be6 }, // Batak Sign Tompi        ,0xatak Sign Tompi
            { 0x01be8, 0x01be9 }, // Batak Vowel Sign Pakpak ,0xatak Vowel Sign Ee
            { 0x01bed, 0x01bed }, // Batak Vowel Sign Karo O ,0xatak Vowel Sign Karo O
            { 0x01bef, 0x01bf1 }, // Batak Vowel Sign U For S,0xatak Consonant Sign H
            ...
            { 0x01dc0, 0x01df9 }, // Combining Dotted Grave A,0xombining Wide Inverted
            { 0x01dfb, 0x01dff }, // Combining Deletion Mark ,0xombining Right Arrowhea
            { 0x020d0, 0x020f0 }, // Combining Left Harpoon A,0xombining Asterisk Above
            { 0x02cef, 0x02cf1 }, // Coptic Combining Ni Abov,0xoptic Combining Spiritu
            { 0x02d7f, 0x02d7f }, // Tifinagh Consonant Joine,Tifinagh Consonant Joine
            { 0x02de0, 0x02dff }, // Combining Cyrillic Lette,0xombining Cyrillic Lette
            { 0x0302a, 0x0302d }, // Ideographic Level Tone M,Ideographic Entering Ton
            { 0x03099, 0x0309a }, // Combining Katakana-hirag,0xombining Katakana-hirag
            { 0x0a66f, 0x0a672 }, // Combining Cyrillic Vzmet,0xombining Cyrillic Thous
            { 0x0a674, 0x0a67d }, // Combining Cyrillic Lette,0xombining Cyrillic Payer
            { 0x0a69e, 0x0a69f }, // Combining Cyrillic Lette,0xombining Cyrillic Lette
            { 0x0a6f0, 0x0a6f1 }, // Bamum Combining Mark Koq,0xamum Combining Mark Tuk
            ...
            { 0x0a8e0, 0x0a8f1 }, // Combining Devanagari Dig,0xombining Devanagari Sig
            { 0x0a8ff, 0x0a8ff }, // Devanagari Vowel Sign Ay,0xevanagari Vowel Sign Ay
            ...
            { 0x0aa29, 0x0aa2e }, // Cham Vowel Sign Aa      ,0xham Vowel Sign Oe
            { 0x0aa31, 0x0aa32 }, // Cham Vowel Sign Au      ,0xham Vowel Sign Ue
            { 0x0aa35, 0x0aa36 }, // Cham Consonant Sign La  ,0xham Consonant Sign Wa
            { 0x0aa43, 0x0aa43 }, // Cham Consonant Sign Fina,0xham Consonant Sign Fina
            { 0x0aa4c, 0x0aa4c }, // Cham Consonant Sign Fina,0xham Consonant Sign Fina
            ...
            { 0x0fe20, 0x0fe2f }, // Combining Ligature Left ,0xombining Cyrillic Titlo
            { 0x101fd, 0x101fd }, // Phaistos Disc Sign Combi,Phaistos Disc Sign Combi
            { 0x102e0, 0x102e0 }, // Coptic Epact Thousands M,0xoptic Epact Thousands M
            { 0x10376, 0x1037a }, // Combining Old Permic Let,0xombining Old Permic Let
            ...
            { 0x11001, 0x11001 }, // Brahmi Sign Anusvara    ,0xrahmi Sign Anusvara
            { 0x11038, 0x11046 }, // Brahmi Vowel Sign Aa    ,0xrahmi Virama
            { 0x1107f, 0x11081 }, // Brahmi Number Joiner    ,Kaithi Sign Anusvara
            { 0x110b3, 0x110b6 }, // Kaithi Vowel Sign U     ,Kaithi Vowel Sign Ai
            { 0x110b9, 0x110ba }, // Kaithi Sign Virama      ,Kaithi Sign Nukta
            { 0x11100, 0x11102 }, // Chakma Sign Candrabindu ,0xhakma Sign Visarga
            { 0x11127, 0x1112b }, // Chakma Vowel Sign A     ,0xhakma Vowel Sign Uu
            { 0x1112d, 0x11134 }, // Chakma Vowel Sign Ai    ,0xhakma Maayyaa
            ...
            { 0x11366, 0x1136c }, // Combining Grantha Digit ,0xombining Grantha Digit
            { 0x11370, 0x11374 }, // Combining Grantha Letter,0xombining Grantha Letter
            ...
            { 0x1171d, 0x1171f }, // Ahom Consonant Sign Medi,0xhom Consonant Sign Medi
            { 0x11722, 0x11725 }, // Ahom Vowel Sign I       ,0xhom Vowel Sign Uu
            { 0x11727, 0x1172b }, // Ahom Vowel Sign Aw      ,0xhom Sign Killer
            { 0x1182f, 0x11837 }, // Dogra Vowel Sign U      ,0xogra Sign Anusvara
            { 0x11839, 0x1183a }, // Dogra Sign Virama       ,0xogra Sign Nukta
            ...
            { 0x11c30, 0x11c36 }, // Bhaiksuki Vowel Sign I  ,0xhaiksuki Vowel Sign Voc
            { 0x11c38, 0x11c3d }, // Bhaiksuki Vowel Sign E  ,0xhaiksuki Sign Anusvara
            { 0x11c3f, 0x11c3f }, // Bhaiksuki Sign Virama   ,0xhaiksuki Sign Virama
            ...
            { 0x16af0, 0x16af4 }, // Bassa Vah Combining High,0xassa Vah Combining High
            { 0x16b30, 0x16b36 }, // Pahawh Hmong Mark Cim Tu,Pahawh Hmong Mark Cim Ta
            { 0x16f4f, 0x16f4f }, // Miao Sign Consonant Modi,Miao Sign Consonant Modi
            { 0x16f8f, 0x16f92 }, // Miao Tone Right         ,Miao Tone Below
            { 0x16fe4, 0x16fe4 }, // (nil)                   ,(nil)
            { 0x1bc9d, 0x1bc9e }, // Duployan Thick Letter Se,0xuployan Double Mark
            { 0x1d167, 0x1d169 }, // Musical Symbol Combining,Musical Symbol Combining
            { 0x1d17b, 0x1d182 }, // Musical Symbol Combining,Musical Symbol Combining
            { 0x1d185, 0x1d18b }, // Musical Symbol Combining,Musical Symbol Combining
            { 0x1d1aa, 0x1d1ad }, // Musical Symbol Combining,Musical Symbol Combining
            { 0x1d242, 0x1d244 }, // Combining Greek Musical ,0xombining Greek Musical
            ...
            { 0x1e000, 0x1e006 }, // Combining Glagolitic Let,0xombining Glagolitic Let
            { 0x1e008, 0x1e018 }, // Combining Glagolitic Let,0xombining Glagolitic Let
            { 0x1e01b, 0x1e021 }, // Combining Glagolitic Let,0xombining Glagolitic Let
            { 0x1e023, 0x1e024 }, // Combining Glagolitic Let,0xombining Glagolitic Let
            { 0x1e026, 0x1e02a }, // Combining Glagolitic Let,0xombining Glagolitic Let
            { 0x1e130, 0x1e136 }, // Nyiakeng Puachue Hmong T,Nyiakeng Puachue Hmong T
            { 0x1e2ec, 0x1e2ef }, // Wancho Tone Tup         ,Wancho Tone Koini
            { 0x1e8d0, 0x1e8d6 }, // Mende Kikakui Combining ,Mende Kikakui Combining
            { 0x1e944, 0x1e94a }, // Adlam Alif Lengthener   ,0xdlam Nukta
