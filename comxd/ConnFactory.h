...
public:
    ...
    SerialConn* CreateInst(const char* type_name, std::shared_ptr<serial::Serial> serial);
    //
    bool RegisterCreateInstFunc(const char* type_name,
                                std::function<SerialConn*(std::shared_ptr<serial::Serial>)> func)
    {
...
protected:
    ...
    std::map<std::string,
        std::function<SerialConn*(std::shared_ptr<serial::Serial>)> > mInstFuncs;
...
public: \
    ...
    { \
        ...
        [=](std::shared_ptr<serial::Serial> serial)->SerialConn* { \
